...
export function CardModal({ card, onClose, onUpdateCard }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 text-white">
      <div className="bg-white p-6 rounded shadow-lg w-full max-w-xl relative flex">
        <div className="flex-1">
          <span
            onClick={onClose}
            className="absolute top-2 right-2 cursor-pointer text-gray-700 hover:text-gray-200"
            role="button"
            aria-label="Cerrar"
          >
            <img src={cerrarSvg} alt="Cerrar" className="w-4 h-4" />
          </span>
          <h3 className="text-2xl font-bold text-gray-900 mb-2">{cardData.nombreActividad || "Nombre de actividad no disponible"}</h3>

          <p className="mb-2 text-gray-600">En la lista: {nombreLista || "Nombre de lista no disponible"}</p>
          ...
          <h4 className="font-semibold mb-1 text-gray-800">Usuario asignado:</h4>
          <p className="text-gray-600">{loading ? "Cargando..." : usuarioAsignado || "Sin usuario asignado"}</p>
          <h4 className="font-semibold mb-1 text-gray-800">Fecha de creación:</h4>
          <p className="text-gray-600">{card.fechaCreacion || "Fecha de creación no disponible"}</p>
          <h4 className="font-semibold mb-1 text-gray-800">Fecha de vencimiento:</h4>
          {fechaVencimiento !== 'Sin fecha de vencimiento' ? (
            <p className={`${new Date(fechaVencimiento) < new Date() ? "text-red-500" : "text-teal-600"}`}>
              {new Date(fechaVencimiento).toISOString().split('T')[0]} {/* Formato YYYY-MM-DD */}
            </p>
          ) : (
            <p className="text-gray-600">{fechaVencimiento}</p>
          )}
        </div>
  ...
  className="bg-blue-600 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded-full ml-4"
>
  Asignar usuario
</button>
...
<button
  ...
  className="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-full ml-4"
>
  Asignar fecha de vencimiento
</button>
<button
  onClick={() => setShowMoveCardModal(true)}
  className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full ml-4"
>
  Mover tarjeta
</button>
<button
